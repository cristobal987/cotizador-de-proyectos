...
function SetsEditForm({data, financeData, type, catalogue, show, onCancel, onSubmit}) {
    const [activeKey, setActiveKey] = useState(1)
    const [nextButtonText, setNextButtonText] = useState("Siguiente")
    const [backButtonText, setBackButtonText] = useState("Cancelar")

    const [concepts, setConcepts ] = useState(data.concepts? data.concepts : {})
    const [profiles, setProfiles ] = useState(data.profiles? data.profiles : [])
    const [finance, setFinanace] = useState(financeData)
    const [costs, setCosts] = useState(data.costs? data.costs : {})
                ...
                <Tabs id="sets-tabs" defaultActiveKey={1} activeKey={activeKey}>
                    <Tab eventKey={1} title={"Conceptos"}>
                        <div style={{padding:"1rem"}}>
                            <SetsAddFormConcepts 
                            conceptsData={data.concepts.concepts}
                    ...
                    <Tab eventKey={2} title={"Perfiles"}>
                        <div style={{padding:"1rem"}}>
                            <SetsAddFormProfile
                                profileData={data.profiles.profiles}
                                type={type}
                                onChange={data => {
                                    setProfiles(data)
                                }}
